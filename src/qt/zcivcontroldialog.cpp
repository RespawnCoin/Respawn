...
std::list<std::string> ZRspControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZRspControlDialog::listMints;

ZRspControlDialog::ZRspControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZRspControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZRspControlDialog::~ZRspControlDialog()
{
    delete ui;
}

void ZRspControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZRspControlDialog::updateList()
{
...
void ZRspControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZRspControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZRsp_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZRspControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZRspControlDialog::GetSelectedMints()
{
...
void ZRspControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
