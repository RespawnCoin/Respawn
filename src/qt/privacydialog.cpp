                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zRSP ought to be enough for anybody." - Bill Gates, 2017
    ui->zRSPpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzRSPSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzRSP->setEnabled(false);
        ui->pushButtonMintzRSP->setToolTip(tr("zRSP is currently disabled due to maintenance."));

        ui->pushButtonSpendzRSP->setEnabled(false);
        ui->pushButtonSpendzRSP->setToolTip(tr("zRSP is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zRSPpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzRSP_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSP is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zRSP...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zRSP in ") +
...
void PrivacyDialog::on_pushButtonSpendzRSP_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zRSP is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zRSP
        sendzRSP();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zRSP
    sendzRSP();
}

void PrivacyDialog::on_pushButtonZRspControl_clicked()
{
    ZRspControlDialog* zRspControl = new ZRspControlDialog(this);
    zRspControl->setModel(walletModel);
    zRspControl->exec();
}

void PrivacyDialog::setZRspControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzRspSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzRSP()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Respawn Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zRSPpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zRSP is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " RSP </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zRSP</b>";
    ...
    // use mints from zRsp selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZRspControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZRspControlDialog::GetSelectedMints();
    }
    ...
    ZRspControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zRsp Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Rsp, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zRsp Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zRSP </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zRSP "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zRSP "));
    ui->labelzRSPAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzRSPSyncStatus->setVisible(fShow);
}
