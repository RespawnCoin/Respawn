...
public:
    CMainParams()
    {
        ...
        nDefaultPort = 5600;
        bnProofOfWorkLimit = ~uint256(0) >> 20; // Respawn starting difficulty is 1 / 2^12
        ...
        nTargetSpacing = 1 * 60;  // Respawn: 1 minute
        nMaturity = 10;
		...
		vSeeds.push_back(CDNSSeedData("nodes.respawn.pw", "nodes.respawn.pw")); // Primary DNS Seeder
		vSeeds.push_back(CDNSSeedData("respawn.nodes.gyservers.com", "respawn.nodes.gyservers.com")); // Secondary DNS Seeder
		vSeeds.push_back(CDNSSeedData("respawn.seeds.mn.zone", "respawn.seeds.mn.zone")); // Third party DNS Seeder
		vSeeds.push_back(CDNSSeedData("respawn.mnseeds.com", "respawn.mnseeds.com")); // Third party DNS Seeder
...
public:
    CTestNetParams()
    {
        ...
        nMinerThreads = 0;
        nTargetTimespan = 1 * 60; // Respawn: 1 day
        nTargetSpacing = 1 * 60;  // Respawn: 1 minute
        nLastPOWBlock = 200;
        nMaturity = 15;
	...
	hashGenesisBlock = genesis.GetHash();
        ...
        vFixedSeeds.clear();
        vSeeds.clear();
        vSeeds.push_back(CDNSSeedData("nodes-test.respawn.pw", "nodes-test.respawn.pw")); // Primary DNS Seeder
        vSeeds.push_back(CDNSSeedData("respawn-test.nodes.gyservers.com", "respawn-test.nodes.gyservers.com")); // Secondary DNS Seeder
        vSeeds.push_back(CDNSSeedData("respawn-test.seeds.mn.zone", "respawn-test.seeds.mn.zone")); // Third party DNS Seeder
        vSeeds.push_back(CDNSSeedData("respawn-test.mnseeds.com", "respawn-test.mnseeds.com")); // Third party DNS Seeder


        base58Prefixes[PUBKEY_ADDRESS] = std::vector<unsigned char>(1, 139); // Testnet respawn addresses start with 'x' or 'y'
        base58Prefixes[SCRIPT_ADDRESS] = std::vector<unsigned char>(1, 19);  // Testnet respawn script addresses start with '8' or '9'
        base58Prefixes[SECRET_KEY] = std::vector<unsigned char>(1, 239);     // Testnet private keys start with '9' or 'c' (Bitcoin defaults)
        // Testnet respawn BIP32 pubkeys start with 'DRKV'
        base58Prefixes[EXT_PUBLIC_KEY] = boost::assign::list_of(0x3a)(0x80)(0x61)(0xa0).convert_to_container<std::vector<unsigned char> >();
        // Testnet respawn BIP32 prvkeys start with 'DRKP'
...
public:
    CRegTestParams()
    {
        ...
        nToCheckBlockUpgradeMajority = 1000;
        nMinerThreads = 1;
        nTargetTimespan = 24 * 60 * 60; // Respawn: 1 day
        nTargetSpacing = 1 * 60;        // Respawn: 1 minutes
