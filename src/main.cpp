...
#if defined(NDEBUG)
#error "Respawn cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zRsp spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zRsp spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zRsp spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 1350000 * COIN;
    } else if (nHeight < 60000 && nHeight > 0) {
        nSubsidy = 28 * COIN;
    } else if (nHeight <= 125000 && nHeight >= 60000) {
        nSubsidy = 20 * COIN;
    } else if (nHeight <= 350000 && nHeight >= 125000) {
		nSubsidy = 16 * COIN;
    } else if (nHeight <= 500000 && nHeight >= 350000) {
		nSubsidy = 12 * COIN;
    } else if (nHeight <= 775000 && nHeight >= 500000) {
		nSubsidy = 8 * COIN;
    } else if (nHeight <= 1025000 && nHeight >= 775000) {
		nSubsidy = 4 * COIN;
    } else if (nHeight <= 1500000 && nHeight >= 1025000) {
		nSubsidy = 2 * COIN;
    } else if (nHeight <= 2620119 && nHeight >= 1500000) {
		nSubsidy = 1 * COIN;	
	} else if (nHeight >= 2620119) {
        nSubsidy = 1 * COIN;
    } else {
        nSubsidy = 1 * COIN;
    }
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Respawn
...
{
    RenameThread("respawn-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZRSPMinted()
{
...
void RecalculateZRSPSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zRSP supply
...
bool RecalculateRSPSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zRsp with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zRspSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert RSP to zRSP
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zRsp spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zRsp serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zRsp mints and %d zRsp spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Respawn: We use certain sporks during IBD, so check to see if they are
